...
//       <div className="p-4 border-b border-[#5a93e8] flex items-center justify-center gap-2">
  ...
  const SidebarContent = () => (
    <div className="w-full h-full flex flex-col bg-[#377dec] text-white">
      ...
      <nav className="flex-1 p-2 overflow-y-auto flex flex-col items-center">
        <ul className="flex flex-col space-y-5 w-full max-w-[150px]">
          {navItems.map((item) => {
            const isActive = pathname === item.href
            const Icon = item.icon
            ...
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={`flex items-center text-bold gap-3 px-3 py-2 rounded-lg transition-colors ${
                    isActive ? "bg-white text-[#377dec]" : "hover:bg-white/10"
                  }`}
                >
                  <Icon size={18} />
